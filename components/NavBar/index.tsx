...
const Logo = ({ bgColor }: { bgColor?: string }): JSX.Element => {
    ...
    <>
      ...
      {currentUser && currentUser.avatar ? (
        ...
        <WalletButton disabled={isLoginDisabled} onClick={connectWallet}>
          <>
            <WalletImg></WalletImg>
            {isDesktop ? <span>Connect Wallet</span> : null}
          </>
        </WalletButton>
      )}
    </>
    ...
    <>
      ...
      {currentUser && currentUser.avatar ? (
        ...
        <WalletButtonnew disabled={isLoginDisabled} onClick={connectWallet}>
          <>
            <WalletImg></WalletImg>
            {isDesktop ? <span>Connect Wallet</span> : <span>Connect Wallet</span>}
          </>
        </WalletButtonnew>
      )}
    </>
  ...
  return (
    <Navbar isBordered className={`${bgColor ?? 'bg-[#fffdf7]'} py-3`} style={{ zIndex: 1000 }}>
      <NavbarContent>
        <NavbarBrand>
          <Link className="font-bold text-inherit" href={'/'}>
            <div className="  flex  justify-end w-full  ">
              ...
              />
              <Image src={images.logo_text} alt="logo" className=" " />
            </div>
          </Link>
        </NavbarBrand>
      </NavbarContent>

      <NavbarContent className="xl:hidden absolute right-5" justify="end">
      ...
      {navbar && (
        <motion.section
          ...
          className="backdrop-blur bg-black/20 w-full h-screen xl:hidden block fixed top-0 right-0 z-50">
          <motion.section
            initial={{ opacity: 0, x: '100%' }}
            animate={{ opacity: 1, x: 0 }}
            exit={{ opacity: 0, x: '100%' }}
            transition={{ duration: 0.3 }}
            className="bg-white md:min-w-[400px] min-w-[300px] h-screen xl:hidden block fixed top-0 right-0">
            ...
            <div className="relative h-screen">
              <section className="top-20 left-0 right-0 flex justify-center relative" style={{ top: '160px' }}>
...
const Dropdownnew = ({
  ...
  return (
    <DropdownListnav isOpen={isOpen} className='bg-white'>
      {currentUser && currentUser.avatar ? (
        <DropdownProfile>
          {/* <AvatarContainer> */}
          {/* <Imagea
