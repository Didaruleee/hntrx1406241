...
const PageHeader = ({
  ...
  return (
    <section>
      ...
      <Modal
        isOpen={!hideFilter}
        className="right-0 absolute w-100 h-full p-5 "
      ...
      <Tabs
        ...
        <Tab key={'/the-den?tab=all'} title="Overview">
          ...
          <div className="flex    ">
            <div className="xll:block hidden lg:w-1/6  ${isFixed ? ' fixed z-42 mb-90 lg:sticky lg:top-0' : ' '}  ">
              <FilterTab
                filterValues={filterValues}
                filterKeys={filterKeys}
                selectValue={selectValue}
            ...
            <div className={`   ${isFixed ? '   ' : ' '} `}>
              <Tabs
                ...
                className={` flex  justify-normal transition-all ease-in-out border  ${
                  isFixed
                    ? ' bg-[#fffdf7] fixed lg: top-20  z-40  lg:py-10 xs:py-5    xs:w-full  lg:pb-6 xs:pb-6  lg:w-full  '
                    : '     '
                }`}
                // className={` flex xs:justify-center   ${isFixed ? '   ' : ' lg:justify-normal '} `}
              >
                <Tab key={'/the-den?tab=all'} title="All">
                  <div className="lg:flex justify-end lg:-mt-12 xs:-mt-11  ${isFixed ? '  xs:flex  ' : '  '}  ">
                    <div
                      className={`lg:mr-9   ${
                        isFixed
                          ? 'bg-[#fffdf7] fixed top-20 lg:py-10 xs:py-12 lg:px-0 xs:px-6 z-40  pb-5 lg:-mt-3 xs:-mt-6    '
                          : ' xs:relative '
                      }`}>
                      <SortTab
                    ...
                    <div
                      className={` lg:hidden   ${isFixed ? '  fixed  top-20  z-40 py-12 -px-6 pb-5 -mt-6' : 'xs:relative '}`}
